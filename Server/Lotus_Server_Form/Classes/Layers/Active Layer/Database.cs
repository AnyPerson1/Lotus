    ...
    public static class Database
    {
        ...
        public static void CreateConnection(string connectionString)
        {
            try
            {
                ...
                connection.Open();
                Logger.Log("Database connection Successful!",Logger.LogLayer.Layer3);
            }
            catch (Exception ex)
            {
        ...
        public static void CreateTableIfNotExists(string tableName)
        {
            ...
            {
                cmd.ExecuteNonQuery();
                Logger.Log($"Table '{tableName}' checked/created.", Logger.LogLayer.Layer3);
            }
            catch (Exception ex)
            {
        ...
        //metod5
        public static void CreateUserTableAndAddUser(string userName, string password, string email, IPAddress ipadress)
        {
            ...
            try
            {
                ...
                // Kullanıcı ekleme
                string insertUserQuery = $"INSERT INTO users (name, password, email) VALUES ('{userName}', '{password}', '{email}',{ipadress.ToString()})";
